...
# Решение без interception вариант 1

b = (np.mean(zp1 * ks) - np.mean(zp1) * np.mean(ks)) / (np.mean(zp1**2) - np.mean(zp1) ** 2)
a = np.mean(ks) - b * np.mean(zp1)
print(f'b0 - {a}')
print(f'b1 - {b}')

# Решение без interception вариант 2
